...
そこで重要になるのは、その後のプロジェクト進行における技術です。いくつか、参考になる記事などをご紹介します。

 : プロジェクトをリードする技術

これには、kakakakakkuさんの「プロジェクトをリードする技術@<fn>{ProjectLeadtech}」がとても参考になります。関連する登壇資料やそのの説明などもたくさんあり、非常に読み応えのある記事です。
...
過去に「第一線のプロがホンネで教える 超実践的 Webディレクターの教科書」を読んだのですが、この本はなかなか面白かったです。
