...
相対見積もりにおいては、ポイントをみんなの共通認識にするためにプランニングポーカーをするといいでしょう。プランニングポーカーとは、見積もりの数値を書いたカードを使って、チーム全員である要件についてのポイントを見積もる方法です。チームメンバーそれぞれが自身の思う見積もりの値を提示しあい、その値にズレが生じた場合は、どこにズレが生じているのか、認識相違がある部分はどこか、を話し合います。話し合いの中でリスクを見つけたしたり、逆に素晴らしい解決策を見つけ出したりできますし、要件の不明瞭な部分を洗い出し、表出化させることもできます。

見積もりの数値には様々な流派があります。好きなものをチームで選んで使うのがよいでしょう。

 * 1, 2, 3, 5, 8, 13, 21, 34, 55 … フィボナッチ数列に従うもの
 * 1, 2, 3, 5, 8, 13, 20, 40, 80 … フィボナッチ数列で大きな数値を丸めたもの
 * 0, 0.5,…, ∞, ？  様々なバリエーションがあります。∞は「規模が大きすぎて見積もれない」、？は「規模見積もりのための情報が不足している」を表明する
