...
否、 @<em>{見積もりをする前にもっとお互いのことを理解して、暖かみのある見積もりをするべきである}というのが本章で伝えたいことです。
...
整理をする観点としては、次のような点が挙げられます。

 * なぜ、そのような業務フローになっているのか？
 * 今回の改修内容に付随して、不足している機能はないのか？
 * 既存のシステムに不要な機能はないのか？　など

これらの項目を確認することで業務フローが整理され、必要な機能が明確になります。

 : 業務内容で困っているポイントを知る
改修依頼ということは、少なくとも現状のシステムでは困っていることがあるはずです。
...
社内のメンバーといえども、ユーザーファーストの考え方を持って何を作るのかを意識すると、暖かみのある見積もりになります。
...
これは、極端な話になりますが、すべての案件を2倍の時間を見積もることで、空きの時間を作ることも不可能ではありません。しかし、これは暖かみのある見積もりであるとは言えません。
自分で決められるからこそ責任を持って依頼者に対して見積もりの妥当性を示す必要があります。
...
お互いの幸せのためにも、要件を固めるところは妥協をしないようにすると良いでしょう。

 : なぜそれだけの工数がかかるのかを説明する
依頼者にとって工数と言うものは少なければ少ないほどよいはずです。
そのため、見積もりの妥当性の説明については基本的に @<em>{何故、そんなに工数がかかるのか？}を説明することになります。
「この機能を実装するためには、これだけの時間がかかります何故ならば〜」ということを丁寧に説明できるようにしておきましょう。
極稀にですが、依頼者が想像していたよりも、短時間で実現することができる機能があります。その場合も @<em>{何故、たったそれだけの工数でできるのか？} ということも説明できるようにしておくと良いとでしょう。
聞かれないと思っていても、依頼をした人のイメージから工数が離れそうなときには丁寧に説明できる様にしておくだけで暖かみのある見積もりになります。
...
要因として考えられることは様々ですが、良くある例としては次のようなことがあげられます。
...
多くの要因において、次の2つの解決策が考えられます。

 * 実装予定の機能を全て実装するために期日を伸ばす
 * 実装予定の機能の一部を削り、必要最低限な機能を実装する

前者の解決策が理想的ではありますが、どんなときでも期日を伸ばすことができるとは限りません。
そういう場合は、機能を削ることで対応をすることを余儀なくされます。
それまでに暖かみのある見積もりを行っている場合は、業務フローの理解などもしていますので、削ることが可能な機能についてもある程度見当をつけて提案をすることができるます。

== まとめ
見積もりとは開発の最初の段階における信頼形成の大事な事柄です。
暖かみのある見積もりを心がけることで、社内での信頼関係が構築されるため、無理なスケジュール設定や無謀な開発を依頼されたりすることが減るでしょう。
